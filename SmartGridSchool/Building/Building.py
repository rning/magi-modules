...
class Building(DispatchAgent): # difference between DispatchAgent and NonBlockingDispatchAgent?
    ...
    def generation(self):
        ...
        if not self.preDawn and not self.dusk:
            ...
            ### MADE CHANGE cosd(self.ELA) to cosd(self.pELA) and with pAZA
            self.moduleCe = math.cos(self.SELA) * sind(self.pELA) * cosd(self.pAZA - self.SAZA * (180 / math.pi)) + math.sin(self.SELA) * cosd(self.pELA)
